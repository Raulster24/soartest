...
async addProductToBasket() {
  const addToBasketButton = await this.page.waitForSelector('button[aria-label="Add to Basket"]', { timeout: 5000 });
...
async verifyCartItemCount(expectedCount) {
  const cartButton = await this.page.waitForSelector('button[aria-label="Show the shopping cart"]', { timeout: 5000 });
